...
public interface MemberDocsController {
    ...
    @ApiResponses({
            @ApiResponse(
                    ...
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = MemberInfoSaveResponse.class)
                    )
            )
    })
    ResponseEntity<MemberInfoSaveResponse> saveMemberInfo(
            @Parameter(hidden = true) @MemberId Long memberId,
            @RequestBody(description = "민감 정보 저장 요청 DTO", required = true)
            MemberInfoSaveRequest request
    );
    ...
    @ApiResponses({
            @ApiResponse(
                    ...
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = MemberInfoUpdateResponse.class)
                    )
            )
    })
    ResponseEntity<MemberInfoUpdateResponse> updateMemberInfo(
            @Parameter(hidden = true) @MemberId Long memberId,
