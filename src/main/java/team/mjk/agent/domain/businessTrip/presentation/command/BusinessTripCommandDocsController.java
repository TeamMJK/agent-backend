...
public interface BusinessTripCommandDocsController {
    ...
    @Operation(summary = "출장 삭제", description = "출장 정보를 삭제합니다.")
    @ApiResponses({
            @ApiResponse(
                    responseCode = "204",
                    description = "출장 삭제 성공"
            )
    })
    ResponseEntity<Void> deleteBusinessTrip(
            @Parameter(hidden = true) Long memberId,
            @Parameter(description = "출장 ID", required = true) Long businessTripId
    );
